...
### 9.5 Seguridad final — hecha (2026-09-07)
...
- ✅ **Invalidación masiva de sesiones** (2026-09-07). `users.token_version` viaja en el access token como claim `tv` y `JwtStrategy` lo compara en cada request — sin query extra, porque ya traía la fila. Lo cierra `AuthService.closeAllSessions()`, que sube la versión **y** revoca los refresh: revocar solo los refresh dejaba el access vivo hasta 15 minutos, así que cambiar la contraseña porque alguien te entró a la cuenta le regalaba un cuarto de hora. Corre en el nuevo `POST /auth/logout-all` ("cerrar sesión en todos los dispositivos"), al cambiar la contraseña y al resetearla. 16 e2e en `sessions.e2e-spec.ts`; verificado mutando los dos cortes: caen 6 y 7 tests.
- ✅ **Rotación del `JWT_SECRET`: no hacía falta construir nada.** Los refresh tokens no son JWT —son filas opacas en la base—, así que el secreto no los toca: cambiarlo mata los access tokens y cada cliente se recupera con un refresh. El procedimiento quedó en el README, con las dos advertencias que sí importan: el front tiene que **serializar** sus refresh (si no, la detección de reuso convierte la rotación en un logout masivo) y con **más de una réplica** el rollout necesita dos pasos, que hoy no están implementados porque hay una sola.
- ✅ **Rate limiting revisado endpoint por endpoint** (2026-09-07). Todos los topes viven ahora en `src/config/throttler.config.ts`, así que "¿qué límites tiene la API?" se contesta abriendo un archivo. El criterio no fue cuánto cuesta servir sino **quién paga el abuso**, y aparecieron cuatro rutas sin tope propio: `POST /auth/refresh` y `POST /auth/logout` (públicas y con un argon2 adentro: amplificador de CPU), `POST /employees/activate` (canje de token público, la misma forma que `reset-password`, y estaba sin el límite que ese sí tenía) y `POST /employees` + `/employees/:id/invitation` (mandan un mail a una casilla ajena firmado por nuestro dominio: el `@Roles` decía quién puede, no cuántas veces). Más `payments/checkout`, que le pega a Mercado Pago. Los e2e derivan los números del config en vez de copiarlos.
- ⚠️ **El tracker es la IP y no puede ser el negocio.** El `ThrottlerGuard` corre antes que el `JwtAuthGuard` —que es lo correcto, no se verifica una firma para después descartar el pedido—, así que cuando decide no hay `request.user`. Leer el `tenantId` del token sin verificarlo sería peor que no limitar. Consecuencia: un negocio abusivo con IPs rotativas no lo frena esto.
- **`GET /appointments/availability` se revisó y se dejó con el límite global, a propósito.** Es el endpoint más caro, pero el panel lo llama mientras el usuario navega el calendario y un tope propio más bajo que 100/min rompería el uso normal antes que un abuso. El público (`/public/:slug/availability`) sí tiene el suyo, que es la superficie expuesta.
